...
sibling = IF(AND('parent (?x) (?y)',
                 ...
                 NOT('(?y) is (?z)')),
             THEN('sibling (?y) (?z)'))
brother = IF(AND(OR('sibling (?x) (?y)',
                    'sibling (?y) (?x)'),
                 'male (?x)'),
             THEN('brother (?x) (?y)'))
sister = IF(AND(OR('sibling (?x) (?y)',
                    'sibling (?y) (?x)'),
                 'female (?x)'),
             THEN('sister (?x) (?y)'))
...
father = IF(AND('parent (?x) (?y)',
                'male (?x)'),
            THEN('father (?x) (?y)'))
son = IF(AND('parent (?y) (?x)',
             'male (?x)'),
         THEN('son (?x) (?y)'))
daughter = IF(AND('parent (?y) (?x)',
             'female (?x)'),
         THEN('daughter (?x) (?y)'))
grandparent = IF(AND('parent (?x) (?y)',
                     'parent (?y) (?z)'),
                 THEN('grandparent (?x) (?z)'))
grandchild = IF('grandparent (?x) (?y)',
                THEN('grandchild (?y) (?x)'))
cousin = IF(AND('parent (?x) (?y)',
                'parent (?a) (?b)',
                'sibling (?x) (?a)'),
            THEN('cousin (?y) (?b)'))
family_rules = [same_person, sibling, brother, sister, mother, father,
                son, daughter, grandparent, grandchild, cousin]
...
WHAT_I_FOUND_BORING = 'writing so many of the family rules was tedious'
